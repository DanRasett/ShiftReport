...
// Получить настройки из Supabase (или по умолчанию)
export const getSettings = async (): Promise<ShiftSettings> => {
  try {
    ...
    if (!error && data) {
      return {
        showWorker: data.show_worker ?? true,
        showDash: data.show_dash ?? true,
        showFact: data.show_fact ?? true,
        showGoodsTaken: data.show_goods_taken ?? true,
        showCashTaken: data.show_cash_taken ?? true,
        showFine: data.show_fine ?? true,
        showOtherExpenses: data.show_other_expenses ?? true,
...
export const saveSettings = async (settings: ShiftSettings): Promise<void> => {
  try {
    await supabase
      ...
      .upsert({
        id: 1,
        show_worker: settings.showWorker,
        show_dash: settings.showDash,
        show_fact: settings.showFact,
        show_goods_taken: settings.showGoodsTaken,
        show_cash_taken: settings.showCashTaken,
        show_fine: settings.showFine,
        show_other_expenses: settings.showOtherExpenses,
...
export default function SettingsScreen() {
  ...
  const items: { key: keyof ShiftSettings; label: string }[] = [
    { key: 'showWorker', label: 'Сотрудник' },
    { key: 'showDash', label: 'Дэш (Терминал)' },
    { key: 'showFact', label: 'Факт' },
    { key: 'showGoodsTaken', label: 'Взято товарами под зарплату' },
    { key: 'showCashTaken', label: 'Взято деньгами из кассы' },
    { key: 'showFine', label: 'Штраф' },
    { key: 'showOtherExpenses', label: 'Прочие расходы' },
    { key: 'showPhoto', label: 'Фото чека' },
  ];
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subtitle}>
        Выберите, какие блоки показывать при сдаче смены (применяется ко всем сотрудникам)
      </Text>
